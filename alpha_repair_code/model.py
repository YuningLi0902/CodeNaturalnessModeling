...
class SpanLM(object):
    def __init__(self, pretrained: str = "", weight=None, batch_size=1, device=None):
        ...
        if device is None:
            ...
            self.device = device
        if 'facebook' in self.pretrained:
...
class BertLM(object):
    def __init__(self, pretrained: str = "", weight=None, batch_size=1, device=None):
        ...
        if device is None:
            ...
            self.device = device
        if 'bert' in self.pretrained and 'codebert' not in self.pretrained:
            self.model = BertForMaskedLM.from_pretrained(pretrained)
            self.infill_ph = "[MASK]"
            self.tokenizer = BertTokenizer.from_pretrained(self.pretrained)
        elif 'codebert' in self.pretrained:
            self.model = RobertaForMaskedLM.from_pretrained(pretrained)
            self.infill_ph = "<mask>"
            self.tokenizer = RobertaTokenizer.from_pretrained(self.pretrained)
        else:
            assert 'bert' in self.pretrained
        self.max_length = self.model.config.to_dict()['max_position_embeddings']
        print("Max length: {}".format(self.max_length))
        self.model = self.model.to(self.device)
        self.batch_size = batch_size
...
class UniXcoder(nn.Module):
    def __init__(self, pretrained: str = "", weight=None, batch_size=1, device=None):
        ...
        self.config = RobertaConfig.from_pretrained(pretrained)
        self.config.is_decoder = True

        self.model = RobertaModel.from_pretrained(pretrained, config=self.config)
    ...
    def generate(self, source_ids, decoder_only=True, eos_id=None, beam_size=5, max_length=64):
        ...
        encoder_output = self.model(source_ids, attention_mask=mask.to(self.device))
        hidden_states_all = []
            ...
            for _ in range(max_length):
                ...
                if _ == 0:
                    ...
                    out = self.model(input_ids,
                                     attention_mask=self.bias[:, context_ids.size(-1):length, :length].to(self.device),
                ...
                hidden_states_current.append(self.lm_head(hidden_states))
            hyp = beam.getHyp(beam.getFinal())
            pred = beam.buildTargetTokens(hyp)[:beam_size]
            pred = [torch.cat([x.view(-1) for x in p] + [zero] * (max_length - len(p))).view(1, -1) for p in pred]
            preds.append(torch.cat(pred, 0).unsqueeze(0))
            hidden_states_all.append(hidden_states_current)
