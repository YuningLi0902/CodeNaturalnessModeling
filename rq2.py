...
from utils import loadModelInstance, extractExistingResults, getPatchPathFromDir, computeEntropyWithContext


def customAPRPatch(args, lm):
    output_file_name = "result_apr_patch_{}.csv".format(
        args.work_mode) if args.output_file_name is None else args.output_file_name
    ...
    with open(output_path, 'a') as f_out:
        if args.work_mode == "partial":
            patch_info_list = getPatchPathFromDir("partial")
        else:
            patch_info_list = getPatchPathFromDir("complete")
...
def main():
    ...
    parser.add_argument("--work_mode", default=None, type=str, required=True,
                        help="The experiment to run: e.g. partial or complete")
    # Other parameters
